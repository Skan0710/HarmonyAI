...
/** Injects the shared line-hover CSS into <head> once, regardless of how many instances render. */
function useLineHoverStyles() {
    React.useEffect(() => {
        if (stylesInjected || document.getElementById('line-hover-styles')) return;
        const tag = document.createElement('style');
        tag.id = 'line-hover-styles';
        tag.textContent = lineHoverStyles;
        document.head.appendChild(tag);
        stylesInjected = true;
    }, []);
}

export interface LineHoverTextProps
...
/**
 * LineHoverText Component
 *
 * Same hover-underline animations as LineHoverLink, but renders a plain
 * <span> instead of an <a> — for use inside router links/buttons where an
 * anchor tag would be invalid or redundant.
 */
export const LineHoverText = React.forwardRef<HTMLSpanElement, LineHoverTextProps>(
...
LineHoverText.displayName = 'LineHoverText';
